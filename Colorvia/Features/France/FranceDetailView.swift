...
struct FranceDetailView: View {
  ...
  var body: some View {
    ScrollView {
      VStack(spacing: 24) {
        ...
        VStack(spacing: 12) {
          NavigationLink(value: AppRoute.franceMap) {
            Label(L10n.text("france_detail.open_map"), systemImage: "map")
              .font(.headline)
          ...
          Button(role: isVisited ? .destructive : nil) {
            if isVisited {
              ...
              Task { await appState.setVisited(true, countryCode: "FR") }
            }
          } label: {
            Text(
              isVisited
    ...
    .confirmationDialog(
      L10n.text("france_detail.remove_prompt"),
      isPresented: $confirmingRemoval,
      titleVisibility: .visible
    ) {
      Button(removalButtonTitle, role: .destructive) {
        Task { await appState.setVisited(false, countryCode: "FR") }
      }
      Button(L10n.text("common.cancel"), role: .cancel) {}
    } message: {
      if appState.visitedFranceDepartmentCount > 0 {
        Text(L10n.removeFranceMessage(appState.visitedFranceDepartmentCount))
      }
    }
  ...
  private var progressCard: some View {
    VStack(spacing: 15) {
      HStack(alignment: .firstTextBaseline) {
        Text(L10n.franceDepartmentFraction(appState.visitedFranceDepartmentCount))
          .font(.system(size: 34, weight: .light, design: .serif))
        Spacer()
        Text(
          L10n.percentage(
            appState.francePercentage.formatted(.number.precision(.fractionLength(1)))
          )
        )
        .font(.headline)
        .foregroundStyle(ColorviaTheme.secondaryInk)
      }
      ProgressView(value: appState.francePercentage, total: 100)
        .tint(appState.mapColor.color)
      Text(L10n.text("france_detail.department_progress"))
        .font(.subheadline)
        .foregroundStyle(ColorviaTheme.secondaryInk)
  ...
  private var isVisited: Bool {
    appState.visitedCodes.contains("FR")
  }

  private var removalButtonTitle: String {
    appState.visitedFranceDepartmentCount > 0
      ? L10n.text("france_detail.remove_all")
      : L10n.text("country_list.remove_visited")
  }
}
